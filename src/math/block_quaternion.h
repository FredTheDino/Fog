...
struct Q {
    ...
    Q operator+(Q q) { return {x + q.x, y + q.y, z + q.z, w + q.w}; }
    ...
    Q operator*(real s) { return {x * s, y * s, z * s, w * s}; }
    ...
    void operator*=(Q o) { *this = (*this) * (o); }

    Q operator/(Q o) { return (*this) * (-o); }
...
real length(Q q) { return sqrt(length_squared(q)); }

Q normalize(Q q) { return q / length(q); }

Q conjugate(Q q) { return {-q.x, -q.y, -q.z, q.w}; }

Q lerp(Q q1, real lerp, Q q2) { return q1 * lerp + (q2 * (1.0f - lerp)); }
...
Q toQ(real roll, real pitch, real yaw) {
    ...
    return {cy * sr * cp - sy * cr * sp, cy * cr * sp + sy * sr * cp,
