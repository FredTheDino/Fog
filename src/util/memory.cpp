...
#define ADVANCED_BY(ptr, size) (MemoryArena *) (((u8 *)(ptr)) + (size))
        ...
        for (u64 i = 0; i < NUM_ARENAS-1; i++) {
            ...
            global_memory.all_regions[i].memory = ADVANCED_BY(all, ARENA_SIZE_IN_BYTES * i);
        }
    ...
    MemoryArena *request_arena() {
        ...
        ASSERT(global_memory.num_free_regions);  // We should never run out of memory.
